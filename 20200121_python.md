...
파이썬에서 -5 부터 256 까지의 id는 동일합니다. (빈번하게 쓰이는 값을 고정하여 객체를 새로 생성하는 비효율을 줄임. 아까워서!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!)
...
b = a				#변수 선언('=')은 메모리에 둥둥떠다니는 값에 이름을 붙이는 것이다.
a is b
True
```
...
type(tp)
tuple
tp[:2] #슬라이싱 가능. 슬라이싱은 원본을 훼손하지 않는다.
```
